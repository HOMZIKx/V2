...
| Activities Hub copy: matching-first LFG order           | `hub-module-ephemeral.ts` menu text                             | **IMPLEMENTATION_ASSUMPTION_REQUIRES_OWNER_REVIEW** | Exact Discord LFG wizard steps, button labels, empty states | YES (copy only) | YES (no new LFG UX flows)              |
...
| DECISION                                       | CURRENT IMPLEMENTATION                          | STATUS                      | OWNER DECISION NEEDED                  | SAFE TO KEEP? | MUST NOT EXPAND? |
| ---------------------------------------------- | ----------------------------------------------- | --------------------------- | -------------------------------------- | ------------- | ---------------- |
| Matching not public post board                 | domain `rankLfgMatch`, Hub copy                 | **ACCEPTED** (#20)          | —                                      | YES           | NO               |
| DM-first match delivery                        | LFG match → `enqueueUserNotification` DISCOVERY | **ACCEPTED** (direction)    | DM copy, opt-out UX                    | YES           | YES (copy/flows) |
| Characters + class/spec + party roles          | LFG search input fields                         | **ACCEPTED** (model)        | Character picker UX                    | YES           | YES (UX)         |
| TANK / BUFF / DPS / FLEX party roles           | `@v2/hub-core` party-role catalog               | **ACCEPTED**                | FLEX behavior in matching              | YES           | YES              |
| Discovery-first (match existing before create) | Hub menu copy + search API                      | **ACCEPTED** (principle)    | Exact step order in Discord wizard     | YES           | YES              |
| Waiting intent / watch pool                    | `lfg_intents` + watches API                     | **FOUNDATION_WIP**          | Intent TTL, cancel UX, pool visibility | YES           | **YES**          |
| Multi-step Discord LFG wizard                  | partial — API exists, full UX incomplete        | **OWNER_DECISION_REQUIRED** | All screens, modals, mobile            | YES (API)     | **YES**          |
...
| Match scoring weights / reasons display        | `rankLfgMatch` score + reasons[]                | **TECHNICAL_ONLY**          | Which reasons shown to user            | YES           | YES              |
| Anti-spam / rate limits for LFG                | minimal                                         | **OWNER_DECISION_REQUIRED** | Limits, cooldown copy                  | YES           | YES              |
| Admin LFG configuration                        | not implemented                                 | **OWNER_DECISION_REQUIRED** | Admin surfaces                         | N/A           | YES              |
| WWW LFG surface                                | not implemented                                 | **OWNER_DECISION_REQUIRED** | Parity rules with Discord              | N/A           | YES              |
