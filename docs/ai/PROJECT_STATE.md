...
- Dalszy audyt Zeabur ujawnił, że `API_GATEWAY_CORS_ORIGINS` i `IDENTITY_TRUSTED_ORIGINS` w aktywnym automatycznym Git snapshotcie nadal zawierały stale `https//v2-web.zeabur.app`. Próba wymuszenia ręcznego `deploy(..., vars)` utworzyła deploymenty, ale spowodowała regresję runtime (`api-gateway=SUSPENDED`, `identity-service=CRASHED`), dlatego ta ścieżka została odrzucona. Ten commit celowo uruchamia znany, stabilny automatyczny Git deployment w celu przywrócenia produkcji; po jego zakończeniu wymagane są env audit i public E2E. Problem trwałego źródła tych dwóch stale originów pozostaje osobnym blockerem operacyjnym i nie będzie oznaczony jako DONE bez dowodu.
