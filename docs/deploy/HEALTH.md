...
| api-gateway           | `/health/live` | configured identity or activity `/health/ready` is 503/timeout | identity-service, activity-service  | Discord (reported, does not fail API live)       |
...
| admin                 | `GET /health`  | process cannot serve the SPA                                   | api-gateway (browser origin)        | —                                                |
