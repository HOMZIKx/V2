...
### Implementation checkpoint — PR #112

Current private-economy implementation is isolated on child branch `feat/private-economy-scope-20260909`, PR #112, targeting the package branch `feat/party-economy-ai-beta-20260909` / PR #111. Do not recreate it on another branch unless PR #112 is gone.

Implemented in PR #112:

- top-level `/economy` route,
- main navigation entry `Ekonomia` using the existing `market` AppSection/icon,
- `Prywatna / Zespół` scope switch,
- team scope links to the existing per-team Economy pages,
- private summary, direct income, costs and history use player-team API only; no private Economy localStorage,
- reserved API workspace alias `private`,
- actual private repository `workspace_id` is generated server-side from the authenticated viewer via SHA-256 and is never chosen by the browser,
- two different authenticated viewers resolve to two different opaque private workspace IDs,
- private and team entries still use the existing durable PostgreSQL Economy tables, so no new storage migration is required,
- private route cannot use team-owner permissions to edit an existing global item catalog entry,
- private history preserves Yang/Won/GEM currency instead of combining unlike currencies as Yang,
- regression test proves per-viewer private workspace isolation.

Validation note: a child PR whose base is another feature branch currently enters the repository-wide `pnpm validate` path and can fail on pre-existing global Prettier debt. The authoritative integration check for this package is the preview-target CI on PR #111 after PR #112 is merged into that feature branch. Do not call PR #112 production-ready until that integration CI and runtime persistence checks pass.
