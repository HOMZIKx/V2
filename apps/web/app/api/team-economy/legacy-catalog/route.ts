...
let cachedItems: readonly LegacyCatalogItem[] | null = null;
...
async function loadLegacyItems(): Promise<readonly LegacyCatalogItem[]> {
  ...
  const byName = new Map<string, LegacyCatalogItem>();
...
export async function GET(request: NextRequest) {
  if (!(await verifiedViewerId(request))) {
    return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
  }

  try {
    const items = await loadLegacyItems();
    return NextResponse.json({ items, count: items.length });
  } catch (error) {
    console.error('team economy legacy catalogue import failed', error);
    return NextResponse.json({ error: 'legacy_catalog_unavailable' }, { status: 502 });
  }
}
