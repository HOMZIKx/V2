...
function normalize(value: string): string {
  return value.toLocaleLowerCase('pl-PL').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9+]+/g, ' ').trim();
}
...
export async function POST(request: NextRequest) {
  ...
  try { parsed = JSON.parse(text) as { items?: RawRecognizedItem[] }; }
  catch { return NextResponse.json({ error: 'ai_invalid_result' }, { status: 502 }); }

  const items = (Array.isArray(parsed.items) ? parsed.items : []).slice(0, 200).map((raw) => {
    const name = typeof raw.name === 'string' && raw.name.trim() ? raw.name.trim() : 'Nieznany przedmiot';
    ...
    return {
      recognizedName: name, quantity, confidence,
      catalogMatch: catalog ? { id: catalog.id, name: catalog.title, category: catalog.category, imageUrl: catalog.sourceImageUrl ?? catalog.imagePath } : null,
    };
  });
