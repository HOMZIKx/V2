    ...
    <AppShell activeSection="maps" viewerName={initialSnapshot.viewerName}>
      <main className="respawn-page" id="main-content">
        ...
        <section className="respawn-workspace">
          ...
          <aside className="panel respawn-party-panel"><header><span className="section-kicker">Party wyprawy</span><h2>{partyOpen ? 'Asteria · podgląd party' : 'Party nieaktywne'}</h2><p>Tryb wspólny jest gotowy w interfejsie. Zapis między urządzeniami wymaga jeszcze API i bota.</p></header><button className={`respawn-party-toggle ${partyOpen ? 'is-on' : ''}`} onClick={() => setPartyOpen((current) => !current)} type="button"><span />{partyOpen ? 'Party aktywne lokalnie' : 'Uruchom podgląd party'}</button><div className="respawn-party-members">{partyMembers.map((member) => <div key={member.name}><span className={`respawn-member-dot is-${member.state === 'online' ? 'online' : member.state === 'offline' ? 'offline' : 'away'}`} /><strong>{member.name}</strong><small>{member.state}</small></div>)}</div><div className="respawn-party-feed"><span>Oznaczenia w tym podglądzie</span>{activeRecords.length ? activeRecords.slice(0, 5).map((record) => <p key={record.key}><b>{record.entity.name}</b> · {record.confirmedBy ?? '—'} · {getRespawnDisplay(record, now).clock}</p>) : <p>Jeszcze brak znaczników na tym kanale.</p>}</div></aside>
        </section>
        <p aria-live="polite" className="respawn-notice">{notice}</p><p className="respawn-data-note">Katalog timerów pochodzi ze starej aplikacji. Ten ekran zapisuje testowe zgłoszenia lokalnie; grafiki map oraz party między urządzeniami wymagają migracji assetów i API/Postgresa.</p>
      </main>
    </AppShell>
  );
}
