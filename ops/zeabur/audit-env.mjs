...
function safeDisplay(key, value) {
  ...
  if (/(?:_URL|_ORIGIN|_ORIGINS|_BASE_URL|_ISSUER|_AUD|_HOST|_PORT|_ENABLED|NODE_ENV|ALLOW_PRODUCTION_CONNECTIONS|_HEADER)$/i.test(key)) {
...
async function gql(query, variables = {}) {
  ...
  if (!response.ok || body?.errors?.length) {
    writeAndExit({ ok: false, error: 'Zeabur GraphQL audit query failed', details: body?.errors?.map(e => e.message) ?? [response.status] }, 1);
  }
  return body.data;
}

function envMap(vars) {
  const map = new Map();
  for (const v of vars) {
    if (!map.has(v.key)) map.set(v.key, []);
    map.get(v.key).push(v.value ?? '');
  }
  return map;
}
...
function value(map, key) {
  const vals = map.get(key) || [];
  return String(vals.at(-1) ?? '').trim();
}

function enabled(map, key, defaultValue = false) {
  const v = value(map, key).toLowerCase();
  if (!v) return defaultValue;
  return ['1','true','yes','on'].includes(v);
}

function requireKeys(findings, service, map, keys, when) {
  for (const key of keys) {
    if (!present(map, key)) findings.push({ severity: 'critical', service, key, issue: `missing ${when}` });
  }
}
...
for (const svc of services) {
  ...
  for (const key of names) {
    ...
    if ((/_ENABLED$/.test(key) || key === 'ALLOW_PRODUCTION_CONNECTIONS') && value(map,key) && !boolValues.has(value(map,key).toLowerCase())) {
      findings.push({ severity: 'critical', service: svc.name, key, issue: 'invalid boolean spelling' });
    }
  }

  if (activeCodeServices.has(svc.name) && svc.gitTrigger?.branchName !== expectedBranch) {
    findings.push({ severity: 'critical', service: svc.name, key: 'git.branch', issue: `drift: ${svc.gitTrigger?.branchName || '(none)'} != ${expectedBranch}` });
  }

  for (const [key, shown] of Object.entries(safeConfig)) {
    if (typeof shown === 'string' && /(?:localhost|127\.0\.0\.1|0\.0\.0\.0)/i.test(shown) && /(?:_URL|_ORIGIN|_ORIGINS|_BASE_URL|_ISSUER|_AUD)$/i.test(key)) {
      findings.push({ severity: 'critical', service: svc.name, key, issue: 'loopback/local URL configured in production' });
    }
  }
  ...
  if (svc.name === 'identity-service') {
    ...
    if (enabled(map,'IDENTITY_INTERNAL_JWT_ENABLED')) {
      requireKeys(findings, svc.name, map, ['IDENTITY_INTERNAL_JWT_ISSUER','IDENTITY_INTERNAL_JWT_KEYRING_JSON','IDENTITY_INTERNAL_JWT_ACTIVE_KID','IDENTITY_INTERNAL_JWT_ISSUE_URL','IDENTITY_SERVICE_CLIENTS_JSON'], 'when internal JWT is enabled');
    } else {
      findings.push({ severity:'critical', service:svc.name, key:'IDENTITY_INTERNAL_JWT_ENABLED', issue:'internal JWT issuance is disabled' });
    }
  }

  if (svc.name === 'player-team-service') {
    requireKeys(findings, svc.name, map, ['PLAYER_TEAM_DATABASE_URL'], 'for persistence');
    if (!enabled(map,'PLAYER_TEAM_INTERNAL_JWT_ENABLED')) findings.push({ severity:'critical', service:svc.name, key:'PLAYER_TEAM_INTERNAL_JWT_ENABLED', issue:'verified internal JWT auth is disabled/missing' });
    if (enabled(map,'PLAYER_TEAM_INTERNAL_JWT_ENABLED')) requireKeys(findings, svc.name, map, ['PLAYER_TEAM_INTERNAL_JWT_ISSUER','PLAYER_TEAM_INTERNAL_JWT_JWKS_URL'], 'when Player Team internal JWT is enabled');
  ...
  reportServices.push({
    ...
    name: svc.name,
    status: svc.status,
    branch: svc.gitTrigger?.branchName ?? null,
    variableCount: svc.variables?.length ?? 0,
    variableNames: names,
    safeConfig,
    domains: (svc.domains || []).map(d => ({ domain:d.domain, status:d.status })),
    ports: svc.ports || [],
    healthCheck: svc.healthCheckV2 || null,
    resourceLimit: svc.resourceLimit || null,
    autoRestart: svc.autoRestart || null,
  });
}
