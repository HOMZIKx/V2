...
const types = await gql(
  token,
  `{ __type(name: "ServiceSpecSourceInput") { inputFields { name type { kind name ofType { name kind ofType { name } } } } } }`,
);
console.log('ServiceSpecSourceInput:', JSON.stringify(types.data?.__type, null, 2));
...
console.log('\nservice:', JSON.stringify(svc.data ?? svc.errors, null, 2));
