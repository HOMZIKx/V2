...
import { PlayerWorkspaceConfigError } from '../config/player-workspace-env.js';
...
export async function loadInboundClientRegistry(
  ...
  try {
    ...
    throw new PlayerWorkspaceConfigError(
      'PLAYER_WORKSPACE_INBOUND_CLIENTS_JSON must be valid JSON',
    );
  }

  const clientsParsed = z.array(inboundClientSchema).safeParse(parsed);
  if (!clientsParsed.success) {
    throw new PlayerWorkspaceConfigError('PLAYER_WORKSPACE_INBOUND_CLIENTS_JSON has invalid shape');
  }
...
export async function verifyInboundAssertion(
  ...
  try {
    ...
    throw new PlayerWorkspaceError('CLIENT_ASSERTION_INVALID', 'Assertion verification failed');
  }

  if ('kid' in payload) {
    throw new PlayerWorkspaceError(
      'CLIENT_ASSERTION_INVALID',
      'kid must not appear in assertion payload',
    );
  }
  ...
  if (Array.isArray(payload.aud)) {
    throw new PlayerWorkspaceError(
      'CLIENT_ASSERTION_INVALID',
      'Assertion aud must be a single string',
    );
  }
  ...
  if (clientId === undefined || jti === undefined || clientId !== keyEntry.clientId) {
    throw new PlayerWorkspaceError('CLIENT_ASSERTION_INVALID', 'Assertion iss/jti invalid');
  }
  if (subject !== clientId) {
    throw new PlayerWorkspaceError('CLIENT_ASSERTION_INVALID', 'Assertion sub must equal iss');
  }
  if (!z.string().uuid().safeParse(jti).success) {
    throw new PlayerWorkspaceError('CLIENT_ASSERTION_INVALID', 'Assertion jti must be a UUID');
  }
