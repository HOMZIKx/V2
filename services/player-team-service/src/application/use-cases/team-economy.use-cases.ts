...
import { type EconomyCurrency, type EconomyDropSessionInput, type EconomyExpenseInput, type TeamEconomyRepositoryPort } from '../../domain/ports/team-economy.port.js';
...
export class TeamEconomyUseCases {
  ...
  private async assertOwner(viewerId: string, workspaceId: string): Promise<void> {
    ...
    if (memberRole(workspace.state, viewerId) !== 'owner') {
      throw new PlayerTeamError('UNAUTHORIZED', 'only workspace owner can edit the global item catalogue');
    }
  }
  ...
  public async createItem(viewerId: string, workspaceId: string, input: { canonicalName: string; category: string; imageUrl?: string | null; alias?: string | null }) {
    await this.workspace(viewerId, workspaceId);
    return this.repository.createItem({ ...input, createdBy: viewerId });
  }

  public async updateItem(viewerId: string, workspaceId: string, input: { itemId: string; canonicalName?: string; category?: string; imageUrl?: string | null; alias?: string | null }) {
    await this.assertOwner(viewerId, workspaceId);
    return this.repository.updateItem({ ...input, updatedBy: viewerId });
  }

  public async addPrice(viewerId: string, workspaceId: string, input: { itemId: string; unitPrice: number; currency: EconomyCurrency }) {
    await this.workspace(viewerId, workspaceId);
    await this.repository.addPrice({ workspaceId, ...input, createdBy: viewerId });
    return { ok: true } as const;
  }

  public async createDrop(viewerId: string, input: Omit<EconomyDropSessionInput, 'createdBy'>) {
  ...
  public async createExpense(viewerId: string, input: Omit<EconomyExpenseInput, 'createdBy'>) {
    await this.workspace(viewerId, input.workspaceId);
  ...
  public async summary(viewerId: string, workspaceId: string, sinceIso?: string) {
    ...
    const totals = currencies.map((currency) => {
      const gross = drops.reduce((sessionTotal, session) => sessionTotal + session.items
        .filter((item) => item.currency === currency)
        .reduce((sum, item) => sum + item.ourQuantity * item.unitPrice, 0), 0);
      const costs = expenses
        .filter((expense) => expense.currency === currency)
        .reduce((sum, expense) => sum + expense.quantity * expense.unitPrice * expense.ourShareBasisPoints / 10_000, 0);
      return { currency, gross, costs, net: gross - costs };
    });
